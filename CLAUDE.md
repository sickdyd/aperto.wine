...
**And the chime is a separate concern from its control, which is the trap here.** The control is one field on the restaurant settings page (`owner/restaurants/edit`, id in `Owner::OrdersHelper::SOUND_TOGGLE_ID`) and appears nowhere in the chrome — it is a preference, set once per device, not a reading like the badge above. Every *other* owner page therefore renders no control at all, the orders board included, which is the one screen this feature exists for. So the controller is mounted on the shell, connects on every owner page, and takes its answer from `localStorage`; the toggle target is only ever a display of that answer, and `render()` over zero targets is a no-op by design. `connect()` accordingly has **no** early return for a page without one. It used to — `if (!this.hasToggleTarget) return`, correct back when the shell rendered a toggle everywhere and the guard only fired outside a restaurant — and left in place it would have armed nothing, repaired no suspended context and answered no `announce()` anywhere but the settings page: silence on every screen anyone is actually watching, with the toggle still reading ON.
