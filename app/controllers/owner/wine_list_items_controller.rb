module Owner
  class WineListItemsController < BaseController
    ...
    before_action :set_wine_list_item, only: :destroy
    ...
    # appended after the existing items in `Wine.in_display_order`. This is the
    ...
    # safe for the same reason it is in #reposition!: both sides are already
    # scoped to @restaurant, so the same-restaurant invariant can't be
    # violated, and the unique index enforces per-list wine uniqueness.
    def create_all
    ...
    def sort
      ...
      raise ActiveRecord::RecordNotFound if @wine_list.wine_list_items.where(id: item_ids).count != item_ids.size
    ...
    # The one-press alternative to dragging every row: sorts the whole list
    # A–Z. The outer key is the wine's colour in the enum's own order, because
    # that is how the editor draws the list — one drop container per colour —
    # and the flat position sequence has to stay consistent with those groups
    # or a later drag would shuffle rows between them. `id` is the final
    # tiebreak so two wines sharing a name land in a stable order.
    def sort_alphabetically
      ordered_ids = @wine_list.wine_list_items.includes(:wine).sort_by { |item|
        [ Wine.colors[item.wine.color], item.wine.name.to_s.downcase, item.id ]
      }.map(&:id)

      reposition!(ordered_ids)
    ...
    # Writes `position = 1..n` over `ordered_ids` in a single UPDATE ... CASE
    # (one round trip) rather than a per-row `update!` (up to 3N queries, with
    # row locks held across N round trips and a deadlock window between
    # concurrent sorts of the same list). Skipping per-row validations here is
    # deliberate and safe: this only ever rewrites `position` on rows already
    # scoped to `@wine_list`, so it can't violate the same-restaurant or
    # per-list wine-uniqueness invariants — those are enforced at create time
    # (see WineListItem#wine_and_list_share_restaurant and the unique index on
    # [wine_list_id, wine_id]).
    #
    # The relation is re-scoped to `ordered_ids`: a row added concurrently is
    # not in the CASE, and a CASE with no ELSE yields NULL for anything it
    # doesn't match — which the NOT NULL column would reject.
    def reposition!(ordered_ids)
      # An empty list has nothing to sort, and an empty CASE is a syntax error.
      return if ordered_ids.empty?

      WineListItem.transaction do
        when_clauses = ordered_ids.map { "WHEN id = ? THEN ?" }.join(" ")
        bindings = ordered_ids.each_with_index.flat_map { |id, index| [ id, index + 1 ] }
        case_sql = WineListItem.sanitize_sql_array(
          [ "CASE #{when_clauses} END", *bindings ]
        )
        @wine_list.wine_list_items.where(id: ordered_ids).update_all("position = #{case_sql}")
      end
    end
    ...
    # Shared response for create/destroy/sort_alphabetically: a turbo_stream
    # re-render of the members container for JS clients, with an html redirect
    # fallback (and matching flash) for no-JS requests.
